...
import type { ValidatedMime } from './exchange-file.types';
...
export class ExchangeFilePolicyService {
  ...
  private looksLikeSql(text: string): boolean {
    ...
    return (
      ...
      /\b(create\s+table|insert\s+into|select\s+.+\s+from|drop\s+table)\b/.test(
        prefix,
      )
    );
  }
  ...
  private mapMimeToExtension(
    mime: ValidatedMime['mime'],
  ): ValidatedMime['ext'] {
    switch (mime) {
      case 'image/jpeg':
        return 'jpg';
